// Ballpit background (ReactBits, inspired by Kevin Levron) — adaptat la vanilla JS
// pentru site static. Nucleul createBallpit e neschimbat; s-a scos doar wrapper-ul React.
import {
  Vector3 as a,
...
// ── Inițializare pe hero ──────────────────────────────────────────────
(function () {
  const canvas = document.getElementById('hero-ballpit-canvas');
  if (!canvas) return;
  // Respectă prefers-reduced-motion — nu porni animația continuă.
  ...
  createBallpit(canvas, {
    ...
    wallBounce: 0.95,
    followCursor: false,
    colors: [0x28d4ff, 0x1592b8, 0xe8edf5]
  });
})();
